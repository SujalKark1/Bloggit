...
using Bloggit.Services;
using Bloggit.ExceptionHandling;
using Bloggit.Notification;
...
{
    throw new ArgumentException("Secret key must not be null or empty.");
}
...
builder.Services.AddTransient<INotificationRepository, Bloggit.Repository.NotificationRepository>();
...
app.UseCors(builder =>
{
    builder.WithOrigins("http://localhost:5173") // Adjust with your React app URL
        .AllowAnyHeader()
        .AllowAnyMethod()
        .AllowCredentials();
});
